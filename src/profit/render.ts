...
function rankingSection(rows: RankRow[]): string {
  const body = rows
    .map(
  ...
  ${explain('Усі пари (індустрія × якість) відсортовані за чистим прибутком на одну робочу сесію. Верхній зелений рядок — найвигідніше. Колонки: без Tycoon і з ним. Сировина для заводів оцінена за <b>ринковою ціною</b> (маржинальний погляд).')}
...
function guide(m: ReportModel): string {
  return `<div class="guide">
    ...
    <ul>
      ...
      <li>Базис цього звіту: сировина заводів — <b>ринкова ціна</b>; найм рахується від <b>${m.salaryBasis === 'user' ? 'твоєї зарплати' : 'середньої по країні'}</b>.</li>
